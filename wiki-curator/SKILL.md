...
1. **三态认识论与收敛三级阀严密判定 (严禁盲目直接降级 Tier 3)**：
   - **Tier 1 (范式迭代 / 跨代突破)**：若新摄入文献系该主题理论的奠基之作、重大跨代突破或最高法定标准，必须刷新 Frontmatter 规程字段、更新置顶视口并升序插入流变时序表。
   - **Tier 2 (重磅突破 / 关键实证 / 填补断层)**：
     - 若新摄入文献填补了该主题流变时序表中的重大年代空白（例如跨度 > 3~5 年的历史断层）；
     - 或系行业/跨国级大规模权威实证调研（如大样本队列、关键工具断层揭示、重要实务反常）；
     - **必须作为 Tier 2 插入流变时序表的对应年份槽位（单调升序，总行数 $\le 8\sim 9$ 行）**，严禁简单粗暴作为 Tier 3 忽略！
   - **Tier 3 (常规同行实证 / 局部微小验证)**：仅当文献确属常规微小应用或重复性佐证时，遵循 **Tier 3 收敛阀**：严禁改动正文与表格，仅在 `## 3. 关联出处与网络` 末尾追加 `[[新增文献]]`。
2. **双向拓扑织网 (Two-Way Topological Weaving)**：
   - 严禁仅向母主题单一方向连线！
   - 必须基于新文献的讨论领域，检查存量相关文献卡片与相关概念卡片，在它们的关联网络中形成**双向互链**。
3. **衍生次要概念收敛为幽灵双链（核心机制）**：
   - 摄取文献中遇到的新名词、新技术，凡非本文献绝对核心突破者，一律在正文中打为 `[[规范名称 (Abbr)]]` 幽灵双链，留待流水线 B 批量孵化。
