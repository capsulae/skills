...
for item in diff_res.diff_queue:
    ...
    # 强制锁定目标卡片路径：严格采用 diff 派发的 target_wiki_hint，严禁探查盘符或自行翻译重命名
    target_wiki = item.target_wiki_hint
...
0. **强制前置参考审阅 (Mandatory Pre-flight Reference Ingestion)**：
   每次执行文献摄取，必须显式调用 `view_file` 审阅 [`references/evidence_levels.md`](references/evidence_levels.md) 与 [`references/three_mode_epistemology.md`](references/three_mode_epistemology.md)，严禁凭先验经验单方面跳过或粗暴降级！
1. **核心要素提炼标准**：
...
2. **构建文献卡片 (路径锁定为 `item.target_wiki_hint`)**：
...
1. **术语圈定与负向排除过滤**：
   提炼 3~5 个特异性高阶核心术语。**通用耗材（移液枪、室温、离心机、EP管、生理盐水、培养皿、吸头等）严禁打链**。
2. **拓扑查词核心约束**：
   - 绝对严禁手写终端临时脚本或在 `wiki/` 中全局 grep，严防触碰 `## 4. 个人手记禁区`；
   - 统一调用 `python .scripts/gatekeeper.py query-terms` 原子批处理命令（单次原子查询 < 5ms），获取成品双链与消歧状态；
   - 命中 `EXISTS`：强制使用返回的成品 wikilink，严禁别名分裂；
   - 命中 `AMBIGUOUS`：显式消歧决策，结合当前文献领域 tags 选取最匹配候选；
...
3. **构建确定性提交命令并执行**：
   ...
   - **L1/L4 集群子卡特权命令（仅限 L1/L4，子卡 $\le 3$ 张且已落盘）**：
     ...
     ```
4. **即摄即刷与挂账合流**：若提交命令携带 `--terms-file`，或回执提示 `"flush_recommended": true`，无条件立即执行 `python .scripts/gatekeeper.py flush-terms`。
...
2. **规范查词与消歧核验**：
   - 通过 `gatekeeper.py query-terms` 核对其规范中英双语名称与别名；
...
### Step B4: 幽灵雷达重扫验证收敛
卡片全部落盘后，再次运行 `python .scripts/scan_ghosts.py --json`，验证本次孵化主题已从候选池出库，断言 `candidates_above_2 == 0`。
...
1. **受控词表挂账合流 (`flush-terms`)**：
   ...
   ```
2. **日志周期归档 (`archive-log`，提示 `log_archive_recommended` 时)**：
   ```bash
   python .scripts/gatekeeper.py archive-log
   ```
3. **物理指标全量回填 (`backfill-metrics`，提示 `backfill_recommended` 时)**：
   ```bash
...
- **Frontmatter 契约**：
  ...
  ---
  ```
- **沙箱隔离公理**：检索工具、雷达脚本 (`scan_ghosts.py`) 与驾驶舱 (`index.md`) **必须严格过滤排除 `status: draft` 页面**，严禁未验证假说污染知识图谱。
